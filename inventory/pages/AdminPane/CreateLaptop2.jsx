...
const [data1, setData1] = useState([]);
  ...
  useEffect(() => {
      fetch('http://localhost:8081/laptops222' )
        .then(res => res.json())
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8081/laptops223/${id}`);
...
{d.image && <img src={displayImage(d.image)} className="w-16 md:w-32 max-w-full max-h-full" alt={d.name} />}
    
              </td>
              <td className="px-6 py-4 font-semibold text-lg text-gray-900 dark:text-white">{d.name}</td>
              <td className="px-6 py-4 text-lg">{d.gen}</td>
              <td className="px-6 py-4 text-lg">{d.sir}</td>
              ...
              <td className="px-6 py-4 text-lg">{d.price}</td>
              <td class="px-6 py-4">
              <div class="flex">
