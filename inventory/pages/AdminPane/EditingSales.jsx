...
const [laps111,setLaps111] = useState(false);
  ...
  const handleDateChange = (e) => {
    ...
    setBook1((prev) => ({ ...prev, date_of_selling: e.target.value }));
    setBook((prev) => ({ ...prev, date_of_selling: e.target.value }));
   console.log(e.target.value);
  };
  ...
  const handleCheckboxChange = (e) => {
    ...
    setBook1((prev) => ({ ...prev, saleStaffApproved: valuegraph }));
    setBook((prev) => ({ ...prev, saleStaffApproved: valuegraph }));
  };
...
// Get the value directly from the input
  ...
  useEffect(() => {
    axios.get(`http://localhost:8081/geteditingsalestaff/${id}`)
    .then(response => {
      ...
      setBook(prevBook => ({
        ...
        date_of_selling: formatDate(resData.date_of_selling),
      }));
    ...
    const handleClick = async (e) => {
      ...
      console.log(book.date_of_order);
        ...
        await axios.put(`http://localhost:8081/posteditingsalestaff/${id}`, book1);
        console.log(id);
        navigate("/");
      } catch (err) {
        console.log(err);
...
<Form.Control  type="text" name="date_of_order" value={book.date_of_order} disabled placeholder="Date of order" />

</Form.Group>

<Form.Label>Date of Selling</Form.Label>
              <Form.Group controlId="orderDate">
               
              <DatePicker   selected={book.date_of_selling}  
             className="form-control"
               />      
                 
          
            <input type="date"  name="date_of_selling" onChange={handleDateChange} value={book.date_of_selling}
             className="w-100"/>
